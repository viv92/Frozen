'''
--- Program implementing the Frozen method with ViT as the image encoder

-- Features:
1. ViT as the image encoder is the only trainable component, while the T5 encoder and decoder are frozen during training
...
def calculate_loss(image_encoder, tokenizer, t5_model, imgs, cap_tokens_dict, device):
    ...
    img_embs = image_encoder.encode(imgs) # img_embs.shape: [batch_size, seq_len_vit, d_model]
    # feed img_embs to t5 encoder to get encoder output
    enc_out = t5_model.encoder(inputs_embeds=img_embs).last_hidden_state # enc_out.shape: [batch_size, seq_len_vit, d_model]
...
if __name__ == '__main__':
    ...
    d_model = 768 # d_model for ViT should be the same as that of T5
    d_k = 64
    d_v = 64
    n_heads = d_model // d_k
    n_layers = 6
    ...
    # hyperparams
    seq_len_t5 = 512 # required to init T5 Tokenizer (should be greater than seq_len_vit)
    batch_size = 4
    lr = 3e-4
    num_epochs = 1000 * 20
    random_seed = 1010
    ...
    checkpoint_path = 'ckpts_frozen_vit/latest.pt' # path to a save and load checkpoint of the trained ViT
    ...
    t5_tokenizer = T5Tokenizer.from_pretrained(t5_model_name, model_max_length=seq_len_t5)
    ...
    for ep in tqdm(range(num_epochs)):
        ...
        imgs, cap_tokens_dict = process_batch(minibatch, t5_tokenizer, img_size, device) # imgs.shape:[batch_size, 3, 64, 64], captions.shape:[batch_size, seq_len_t5]
